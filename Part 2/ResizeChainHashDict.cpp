...
void ResizeChainHashDict::rehash() {
  ...
  for (int i = 0; i < prevSize; i++) {
		ChainNode * oldNode = table[i];

		while (oldNode != NULL) {
			ChainNode* newNode = new ChainNode();
			newNode->key = oldNode->key; newNode->keyID = oldNode->keyID;
			newNode->data = oldNode->data;
			newNode->next = NULL;
			
			if (newTable[hash(oldNode->keyID)] == NULL) {				
				newTable[hash(oldNode->keyID)] = newNode;
			}
			else {
				ChainNode* newerNode = newTable[hash(oldNode->keyID)];
				newNode->next = newerNode;				
				newTable[hash(oldNode->keyID)] = newNode;
			}
			
			ChainNode* temp = oldNode;
...
bool ResizeChainHashDict::find_helper(string keyID, PuzzleState *&pred) {
	...
	ChainNode * node = table[h];

	while (node != NULL) {
		if (keyID == node->keyID) {
			pred = node->data;
			
			if (numProbes < MAX_STATS) 
				probes_stats[numProbes]++;
